...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($discount_value <= 0) {
        $errors[] = 'İndirim değeri 0\'dan büyük olmalıdır.';
    }
    if ($discount_type === 'percentage' && $discount_value > 100) {
        $errors[] = 'Yüzde indirimi 100\'den fazla olamaz.';
    }
    if ($usage_limit < 1) {
        $errors[] = 'Kullanım limiti en az 1 olmalıdır.';
    }
    if (!empty($valid_until) && strtotime($valid_until) <= time()) {
        $errors[] = 'Bitiş tarihi gelecekte olmalıdır.';
    }
    ...
    if (empty($errors)) {
        if (!empty($_POST['id'])) {
            ...
            $id = $_POST['id'];
            $stmt = $db->prepare('UPDATE Coupons SET code = ?, discount_type = ?, discount_value = ?, min_amount = ?, usage_limit = ?, valid_from = ?, valid_until = ?, is_active = ? WHERE id = ?');
            try {
                $stmt->execute([$code, $discount_type, $discount_value, $min_amount, $usage_limit, $valid_from ?: null, $valid_until ?: null, $is_active, $id]);
            ...
            // Create new coupon
            $id = uniqid('CPN');
            $stmt = $db->prepare('INSERT INTO Coupons (id, code, discount_type, discount_value, min_amount, usage_limit, used_count, valid_from, valid_until, is_active) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?)');
            try {
                $stmt->execute([$id, $code, $discount_type, $discount_value, $min_amount, $usage_limit, $valid_from ?: null, $valid_until ?: null, $is_active]);
...
$stmt = $db->prepare("
    SELECT c.*, 
           COUNT(uc.id) as actual_used_count
    FROM Coupons c 
